...
export default function PendingPickupsView({ 
  ...
  React.useEffect(() => {
    if (activeSubTab === 'ruta' && mapContainer && !mapRef.current && pendingPickups.length > 0) {
      ...
      if (mapboxToken && mapboxToken !== 'pk.eyJ1IjoiZXhhbXBsZSIsImEiOiJjbGV4YW1wbGUifQ.example') {
        ...
        // Add pending pickup markers after map loads
        newMap.on('load', () => {
          setTimeout(() => {
  ...
  }, [activeSubTab, mapContainer, pendingPickups.length, optimizedRoute]);

  // Separate effect to handle map updates when route changes
  React.useEffect(() => {
    if (mapRef.current && activeSubTab === 'ruta' && pendingPickups.length > 0) {
      // Update route when pendingPickups change
      const routeCoordinates: [number, number][] = [
        [-99.1332, 19.4326] // Start from user location
      ];
      ...
      // Update route source if it exists
  ...
  }, [optimizedRoute, pendingPickups.length, activeSubTab]);
